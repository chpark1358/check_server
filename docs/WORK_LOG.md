...
변경 사항:
- 시리얼별 자동 매칭 저장 구조는 유지했습니다.
- 일괄 점검 수동 연결 선택 목록에서 고객사명, Zendesk 조직, 요청자가 동일한 항목의 반복 표시를 제거했습니다.
- 요청자가 다른 동일 고객사를 구분할 수 있도록 선택 항목에 요청자 이메일을 표시했습니다.
- 현재 선택된 시리얼별 매핑이 중복 제거된 대표 항목으로 정상 표시되도록 선택값을 정규화했습니다.
